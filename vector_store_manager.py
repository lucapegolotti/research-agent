...
def chunk_text_by_sentences(text: str, min_sentences: int, max_sentences: int) -> list[str]:
    ...
    try:
        ...
        nltk.download('punkt', quiet=True) # quiet=True to avoid verbose output during chunking
    except Exception as e:
...
def process_single_pdf(pdf_path: str, sentence_model: SentenceTransformer, start_global_chunk_id: int) -> Tuple[Optional[np.ndarray], List[Dict]]:
    ...
    if not raw_text:
        print(f"  Skipping {filename} (dynamic add): extraction error or empty content.")
        return None, []
    ...
    chunk_embeddings_np = sentence_model.encode(chunks, show_progress_bar=False, convert_to_numpy=True)
...
def build_initial_vector_store():
    ...
    if not os.path.exists(PAPERS_DIR) or not os.listdir(PAPERS_DIR):
        print(f"Error: Papers directory '{PAPERS_DIR}' is empty or not found. "
    ...
    for i, filename in enumerate(pdf_files):
        ...
        chunk_embeddings_np = model.encode(chunks, show_progress_bar=False, convert_to_numpy=True)
    ...
    if not all_text_chunks_with_metadata:
        print("No text chunks were generated from any PDF during initial build. Exiting.")
        return
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print(f"Total processing time for initial build: {end_time - start_time:.2f} seconds.")